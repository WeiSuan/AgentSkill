    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=(
                ...
                "再呼叫對應 function tool。分析此問題必須至少使用 stock_price 和 "
                "get_information 各一次。兩個 observation 都具備後，停止呼叫工具，"
...
def main() -> None:
    ...
    available_tools = [
        tool for tool in TOOLS if tool.__name__ != withheld_tool
    ]
    ...
    for step in range(1, MAX_AGENT_STEPS + 1):
        ...
        for function_call in function_calls:
            ...
            elif tool_name in executed_tools:
                observation = {
                    "skipped": True,
                    "reason": f"{tool_name} 已執行過，請改用尚未執行的工具或產生結論。",
                }
            elif arguments:
                observation = {"error": f"{tool_name} 不接受參數：{arguments}"}
            else:
                observation = {"result": TOOL_FUNCTIONS[tool_name]()}
